...
const PaymentsEvent = {
    ...
    getByCreatorId: (userId) => {
        const query = `SELECT payments.id AS payments_id, payments.user_id, payments.amount, payments.recieved_status, payments.paid_status, payments_event.id AS payments_event_id, payments_event.event_name, payments_event.event_creator_id, payments_event.total_amount, payments_event.creation_date, payments_event.due_date FROM payments_event INNER JOIN payments ON (payments_event.id = payments.payment_event_id) WHERE event_creator_id = $1`;
